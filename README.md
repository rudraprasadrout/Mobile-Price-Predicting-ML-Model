...
Frontend UI: mobiprice.netlify.app

API Endpoint: mobile-price-predictor-ml-model.onrender.com

🏗️ System Architecture

The project follows a Decoupled Architecture to ensure scalability and separation of concerns:

Frontend (Netlify): A modern, responsive HTML/JS interface that collects user input and communicates with the backend via REST API calls.

Backend API (Render): A Flask-based server that loads the serialized .pkl models to process real-time prediction requests.

ML Core: A robust pipeline that handles data cleaning, feature engineering, and classification logic.
...
git clone [https://github.com/routrp07/mobile-price-prediction.git](https://github.com/routrp07/mobile-price-prediction.git)
